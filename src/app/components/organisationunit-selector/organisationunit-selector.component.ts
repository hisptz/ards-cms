// import {
//   Component, OnInit, Input, OnChanges, Output, EventEmitter, ChangeDetectionStrategy,
...
//   @Output() onOrgUnitSelectionChange: EventEmitter<any> = new EventEmitter<any>();
...
//         this.onOrgUnitSelectionChange.emit({selectedOrgUnit: this.selectedOrganisationUnits, selectedChildrenLevel: this.selectedChildrenLevel});
//       }
//       this.organisationUnitLevels$.subscribe((organisationUnitLevels: any[]) => {
...
//             this.orgunitChildrenLevels = this.getOrgUnitChildrenLevel(this.organisationUnitLevels, this.selectedOrganisationUnits, this.lowestAssignedLevel);
...
//     this.selectedOrganisationUnits.splice(this.selectedOrganisationUnits.indexOf(event.node.data),1);
//     this.onOrgUnitSelectionChange.emit({selectedOrgUnit: this.selectedOrganisationUnits, selectedChildrenLevel: this.selectedChildrenLevel});
//   }
//
...
//   }
//
//   getOrgUnitChildrenLevel(orgunitLevels, selectedOrgunit, lowestAssignedLevel): any {
...
//     }
//   }
//   return childrenLevel.filter(levelDetail => { return levelDetail.level <= lowestAssignedLevel});
//   }
//
...
//       childrenName = 'Show ';
//       childrenName += childrenLevel.name === 'District' ? 'Regions and ': childrenLevel.name === 'Ward' ? 'Districts and ' : '';
//       childrenName += childrenLevel.name == 'Regional' ? 'Region' : childrenLevel.name
//       childrenName += 's';
