...
$(document).ready(function() {
  ...
  $.getJSON("../../../api/reportTables.json", function(data) {
    ...
    $.each(data.reportTables, function(key, value) {
      var arr = value ? (value.name ? value.name.split(":") : null) : null;
      if (arr) {
        if (arr.length != 1) {
    ...
    for (var i = 0; i < menuarr.length; i++) {
      ...
      menu +=
        '<h6 class="panel-title" style="padding: 5px 5px;font-size: 10pt">';
      menu +=
        '<a class="text-success accordion-toggle" data-toggle="collapse" data-parent="#accordion1" href="#collapseOneItem' +
        i +
        '">';
      menu +=
        '<i class="fa fa-chevron-circle-down pull-right"></i>' + menuarr[i];
      menu += "</a>";
      menu += "</h6>";
      menu += "</div>";
      menu +=
        '<div id="collapseOneItem' + i + '" class="accordion-body collapse">';
      menu += '<div class="accordion-inner">';
      menu += '<ul class="nav nav-pills nav-stacked">';
      $.each(data.reportTables, function(key, value) {
        var arr = value ? (value.name ? value.name.split(":") : []) : [];
        if (arr.length != 1) {
          if (menuarr[i] == arr[0]) {
            menu +=
              '<li style="padding: 8px 4px" class="menuitem listmenu" allstring="' +
              arr[1] +
              '"><a style="padding-left: 0px; padding-right: 0px" href="#" id="' +
              value.id +
              '" class="text-success"></i>' +
              arr[1].substring(0, 20) +
              '...<i class="fa fa-chevron-right pull-right"></i> </a></li>';
          }
        }
      });
      menu += "</ul>";
      ...
      menu1 +=
        '<h6 class="panel-title" style="padding: 5px 5px;font-size: 10pt">';
      menu1 +=
        '<a class="text-success accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapseTwoItem' +
        i +
        '">';
      menu1 +=
        '<i class="fa fa-chevron-circle-down pull-right"></i>' + menuarr[i];
      ...
      menu1 += "</div>";
      menu1 +=
        '<div id="collapseTwoItem' + i + '" class="accordion-body collapse">';
    ...
    $(".menuitem").click(function() {
      ...
      $.getJSON(url1, function(data) {
        ...
        var allunitsOptions = "";
        $.getJSON(
          "../dhis-web-commons/analysis/organisationUnits.json",
          function(data) {
            $.each(data.organisationUnits, function(key, value) {
              //populate regions
              if (value.level == 2) {
